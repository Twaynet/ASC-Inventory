'use client';

import { useAuth } from '@/lib/auth';
import { Header } from '@/app/components/Header';

export default function HelpPage() {
  useAuth();

  return (
    <>
      <Header title="Help & FAQ" />
      <main className="container-full py-8 px-6 max-w-[800px] mx-auto">

        <section className="mb-10">
          <h2 className="text-xl text-text-primary border-b-2 border-border pb-2 m-0 mb-4">Getting Started</h2>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I log in?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Enter your <strong>Facility Key</strong>, <strong>Username</strong>, and <strong>Password</strong> on the login page. Your facility key identifies your surgery center — your administrator can provide it if you don&apos;t have it.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">What is the Dashboard?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>The Dashboard is your home screen. It shows feature cards based on your role — only the features you have access to will appear. Click any card to navigate to that section.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">What are the different user roles?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6]">
              <table className="w-full border-collapse text-[0.85rem] mt-2">
                <thead>
                  <tr>
                    <th className="py-2 px-3 text-left border-b border-border bg-surface-tertiary font-semibold text-text-primary">Role</th>
                    <th className="py-2 px-3 text-left border-b border-border bg-surface-tertiary font-semibold text-text-primary">Description</th>
                  </tr>
                </thead>
                <tbody className="[&_td]:py-2 [&_td]:px-3 [&_td]:border-b [&_td]:border-border [&_td:first-child]:font-medium [&_td:first-child]:whitespace-nowrap">
                  <tr><td>Admin</td><td>Full access to all features including user management, settings, and reports</td></tr>
                  <tr><td>Scheduler</td><td>Manage the calendar, approve and schedule cases, assign rooms</td></tr>
                  <tr><td>Inventory Tech</td><td>Manage inventory, check in items, handle the risk queue</td></tr>
                  <tr><td>Circulator</td><td>View cases, complete surgical timeouts and debriefs</td></tr>
                  <tr><td>Scrub</td><td>View cases, sign off on timeouts and debriefs</td></tr>
                  <tr><td>Surgeon</td><td>View your cases, manage preference cards, sign debriefs</td></tr>
                  <tr><td>Anesthesia</td><td>View cases and participate in OR workflows</td></tr>
                </tbody>
              </table>
            </div>
          </details>
        </section>

        <section className="mb-10">
          <h2 className="text-xl text-text-primary border-b-2 border-border pb-2 m-0 mb-4">Calendar</h2>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I switch calendar views?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Use the <strong>Month</strong>, <strong>Week</strong>, and <strong>Day</strong> buttons at the top of the calendar to switch views. You can also click on a specific date in Month view to jump to that day.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">What do the colored dots on cases mean?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_ul]:my-2 [&_ul]:pl-5 [&_li]:mb-1.5">
              <ul>
                <li><span className="inline-block w-3 h-3 rounded-full mr-1 align-middle bg-[#22c55e]"></span> <strong>Green</strong> — All required items are available. The case is ready.</li>
                <li><span className="inline-block w-3 h-3 rounded-full mr-1 align-middle bg-[#f97316]"></span> <strong>Orange</strong> — Some items are missing or pending. Needs attention.</li>
                <li><span className="inline-block w-3 h-3 rounded-full mr-1 align-middle bg-[#ef4444]"></span> <strong>Red</strong> — Critical items are missing. The case is not ready.</li>
              </ul>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I view case details from the calendar?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Click on any case in the calendar to open its details. From Day view, you can see all cases for that day organized by time or by operating room.</p>
            </div>
          </details>
        </section>

        <section className="mb-10">
          <h2 className="text-xl text-text-primary border-b-2 border-border pb-2 m-0 mb-4">Cases</h2>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I request a new case?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Navigate to <strong>Case Requests</strong> from the Dashboard. Click <strong>New Case Request</strong> and fill in the surgeon, procedure, preferred date/time, and any notes. The request will be sent to a scheduler for approval.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">What is the case lifecycle?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_ol]:my-2 [&_ol]:pl-5 [&_li]:mb-1.5">
              <ol>
                <li><strong>Requested</strong> — A case has been submitted and awaits scheduler approval.</li>
                <li><strong>Scheduled</strong> — The case is approved and placed on the calendar.</li>
                <li><strong>Ready</strong> — All required items and preparations are confirmed.</li>
                <li><strong>In Progress</strong> — The procedure is underway.</li>
                <li><strong>Completed</strong> — The procedure is finished and documented.</li>
              </ol>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I check if a case is ready?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Open the case to see its readiness dashboard. Each required item from the surgeon&apos;s preference card is listed with its availability status. Green means available, red means missing.</p>
            </div>
          </details>
        </section>

        <section className="mb-10">
          <h2 className="text-xl text-text-primary border-b-2 border-border pb-2 m-0 mb-4">Surgeon Preference Cards</h2>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">What is a Preference Card?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>A Preference Card documents everything a surgeon needs for a specific procedure — instruments, equipment, supplies, medications, positioning preferences, and special notes. It ensures consistent OR setup every time.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I view or edit a Preference Card?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Go to <strong>Preference Cards</strong> from the Dashboard. Select a surgeon and procedure to view the card. If you have edit access, you can update any section. Changes are tracked with version history.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">Can I create a new Preference Card from an existing one?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Yes. Open an existing card and use the <strong>Clone</strong> feature to create a copy. You can then modify it for a different procedure or surgeon.</p>
            </div>
          </details>
        </section>

        <section className="mb-10">
          <h2 className="text-xl text-text-primary border-b-2 border-border pb-2 m-0 mb-4">OR Workflows</h2>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">What is the Surgical Timeout?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>The Surgical Timeout is a safety checklist completed before the procedure begins. It verifies patient identity, procedure details, and required equipment. All team members must confirm before proceeding.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">What is the Debrief?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>The Debrief is completed after the procedure. It documents what happened, any issues encountered, and collects signatures from the OR team (circulator, scrub, surgeon). Staff who have left can sign asynchronously via <strong>Pending Reviews</strong>.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">What if someone missed signing a Debrief?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Outstanding signatures appear in <strong>Pending Reviews</strong> on the Dashboard. The staff member can sign from there at any time.</p>
            </div>
          </details>
        </section>

        <section className="mb-10">
          <h2 className="text-xl text-text-primary border-b-2 border-border pb-2 m-0 mb-4">Admin Features</h2>
          <p className="text-[0.85rem] text-text-muted italic m-0 mb-4">These features are available to users with the Admin role.</p>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I manage users?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Go to <strong>Admin &gt; Users</strong>. You can add new users, assign roles, and activate or deactivate accounts. Users can hold multiple roles.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I approve case requests?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Pending case requests appear in <strong>Case Requests</strong> with a badge count on the Dashboard. Open the request, review the details, assign a room and time, then approve it to add it to the calendar.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I assign operating rooms?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Rooms can be assigned when approving a case request, or later from the case details. Go to <strong>Unassigned Cases</strong> to see all cases that still need a room assignment.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">How do I manage inventory and catalog?</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Use <strong>Admin &gt; Catalog</strong> to organize items into groups and sets. Use <strong>Admin &gt; Inventory</strong> to track stock levels, check in new items, and manage the risk queue for flagged items.</p>
            </div>
          </details>
        </section>

        <section className="mb-10">
          <h2 className="text-xl text-text-primary border-b-2 border-border pb-2 m-0 mb-4">Troubleshooting</h2>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">I can&apos;t see certain features on my Dashboard</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>The Dashboard only shows features your role has access to. If you need access to additional features, contact your administrator to update your role.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">I&apos;m getting &quot;Facility not found&quot; when logging in</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Make sure the Facility Key field matches your surgery center&apos;s key exactly. It is case-sensitive. Contact your administrator if you&apos;re unsure of the correct key.</p>
            </div>
          </details>

          <details className="group border border-border rounded-lg mb-2 bg-surface-primary">
            <summary className="py-3.5 px-4 font-medium text-text-primary cursor-pointer list-none flex items-center gap-2 [&::-webkit-details-marker]:hidden before:content-['▶'] before:text-[0.7rem] before:text-text-muted before:transition-transform before:duration-150 group-open:before:rotate-90 hover:text-accent">A case shows as &quot;not ready&quot; but I think everything is available</summary>
            <div className="px-4 pb-4 pl-7 text-text-secondary text-[0.9rem] leading-[1.6] [&_p]:m-0 [&_p]:mb-2 [&_p:last-child]:mb-0">
              <p>Open the case details and check each line item. Items must be checked in through inventory to register as available. If an item was recently received, it may need to be checked in by an Inventory Tech.</p>
            </div>
          </details>
        </section>

      </main>
    </>
  );
}

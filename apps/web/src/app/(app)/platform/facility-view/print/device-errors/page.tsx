...
import { getDeviceEvents } from '@/lib/api/platform-facility-view';
import type { DeviceEventListItem } from '@/lib/api/inventory';
...
function Content() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const errorEvents = await getDeviceEvents(token, facilityId, { start: startISO, end: endISO, hasError: true, limit: MAX_ERRORS });
        ...
        <>
          <PrintSection title="Summary">
            <table className="print-table w-full text-sm border-collapse border border-border max-w-md">
              <tbody>
                <tr><td className="p-2 border border-border">Error Events (7d)</td><td className="p-2 border border-border text-right font-semibold">{errors.length}</td></tr>
              </tbody>
            </table>
          </PrintSection>

  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [healthR, agingR, locR, catR, devR, finR] = await Promise.all([
        getHealthSummary(token, facilityId, { start: startISO, end: endISO }),
        getOpenMissingAging(token, facilityId),
        ...
        getDeviceEvents(token, facilityId, { start: dev7Start, end: dev7End, hasError: true, limit: DEVICE_ERROR_LIMIT }),
        getInventoryEvents(token, facilityId, { financial: true, start: `${start}T00:00:00Z`, end: `${end}T23:59:59Z`, limit: FINANCIAL_FETCH_LIMIT }),
      ]);

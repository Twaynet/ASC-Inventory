...
// ---------------------------------------------------------------------------
// Pure query helper — no request/reply, no auth, no side effects.
...
export async function queryHealthSummary(
  facilityId: string,
  ...
  const endDate = filters.end ? new Date(filters.end) : now;
  const startDate = filters.start ? new Date(filters.start) : thirtyDaysAgo;
...
export async function operationsHealthRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  registerContractRoute(fastify, contract.operations.healthSummary, PREFIX, {
    ...
    handler: async (request, reply) => {
      ...
      const result = await queryHealthSummary(facilityId!, { start: q.start, end: q.end });
